...
Our recent utility of TensorFlow led us to a tutorial of an RNN model which began our journey. Several attempts were made using our smallest datasets to effectively use TF and Keras to train on varying RNN loops, and an LSTM (not appearing in this summary), before increasing the training set size. This is our journey in an attempt to create a "next-char-predition" model:
...
* Workbooks:
	* [/scripts/pt_bert_bot](https://nbviewer.org/github/famndox/P4-James-Angels/blob/main/Scripts/pt_bert_bot.ipynb): was able to complete the analysis and 12 epochs of training, in about 20 hours, running 
 on a local cpu (jupyterlab + tensorflow)
	* [/scripts/P4_BERT_FINAL_Google_Colab](https://nbviewer.org/github/famndox/P4-James-Angels/blob/main/Scripts/P4_BERT_FINAL_Google_Colab.ipynb): was able to complete the analysis and 15 epochs of training, in about 2 hours, running on Google Colab
* Hugging Face Models:
...
#### Best Results
* From [/scripts/pt_bert_bot](https://nbviewer.org/github/famndox/P4-James-Angels/blob/main/Scripts/pt_bert_bot.ipynb)... Input prompts are highlighted in these screenshots to distinguish input from generation:
 ...
 * From [/scripts/P4_BERT_FINAL_Google_Colab](https://nbviewer.org/github/famndox/P4-James-Angels/blob/main/Scripts/P4_BERT_FINAL_Google_Colab.ipynb)... Oddly enough, although the majority of the training stories were positive, the results yielded a story considered to be very negative:
...
The segment of code performed sentiment analysis on the folktales dataset by calculating the polarity and subjectivity for each tale, then grouped the data by nation to compute the average sentiment scores, and finally sorted the nations by their average polarity for review.
...
## Tableau
### Scope
The aim was to use language models to identify key words that may appear in both the Folk and Grimm tales. By identifying a self-selected list of words for Folk and Grimm tales, we are able to assist our machine model to increase the likelihood that key words are used in the story output. Link to Tableau: https://public.tableau.com/views/Project4Viz_17252868607620/SelectFolkWordsbyNation?:language=en-US&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link 

#### Findings located in Story Word Frequency Findings
1. Notebooks: Folk Word Frequency and Grimm Word Frequency  include the data cleaning for both datasets. The Folk dataset included "nations" which assisted in crafting the tale map that will be discussed later. Both Folk and Grimm datasets required an nltk.tokenizer (Natural Language) to locate strings of words in both sets.  A lemmatizer was used in both Notebooks to identify root words and to further increase the likelihood that words chosen kept their original meanings. An arbitary list of common words were pulled for each dataset and then compiled into a dictionary in order to count the frequency of word usage throughout both datasets. Folk resulted in a more positive and fairy tale-esque list, while the Grimm dataset was more gritty and dark. For the Folk dataset, a second list was crafted to identify the words in the dictionary as it relates to the nation. For example, the number of occurrences of the word "good" differed from Japan to Ireland. 
...
3. Folk Word Frequency: word cloud created from the word dictionary and number of occurrences through all texts.
...
5. Grimm Word Frequency: word cloud created from the word dictionary and number of occurrences through all texts.
